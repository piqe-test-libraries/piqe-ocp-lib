    ...
    lr_body = {
        ...
        "spec": {
            "limits": [
                {
                    "type": "Container",
                    "max": {"cpu": "1200M", "memory": "1Gi"},
                    "min": {"memory": "528Mi", "cpu": "200M"},
                }
            ]
        },
    }
...
class TestOcpLimitRanges:
    def test_container_limit_range_item_builder(self, ocp_lr):
        ...
        if not lr_item.min.get("memory") == "512Mb" and not lr_item.type == "Container":
    ...
    def test_pod_limit_range_item_builder(self, ocp_lr):
        ...
        logger.info(f"Create Response : {lr_item}")
        if not lr_item.min.get("cpu") == "200M" and not lr_item.type == "Pod":
    ...
    def test_image_limit_range_item_builder(self, ocp_lr):
        ...
        logger.info(f"Create Response : {lr_item}")
        if not lr_item.max.get("storage") == "1G" and not lr_item.type == "openshift.io/Image":
    ...
    def test_image_stream_limit_range_item_builder(self, ocp_lr):
        ...
        logger.info(f"Create Response : {lr_item}")
        if not lr_item.max.get("images") == "10" and not lr_item.type == "openshift.io/ImageStream":
    ...
    def test_persistent_volume_claim_limit_range_item_builder(self, ocp_lr):
        ...
        logger.info(f"Create Response : {lr_item}")
        if not lr_item.max.get("storage") == "10G" and not lr_item.type == "PersistentVolumeClaim":
    ...
    def test_build_limit_range(self, ocp_lr):
        ...
        if not isinstance(lr.spec.limits, list) and len(lr.spec.limits) != 2:
    ...
    def test_create_limit_range(self, ocp_lr):
        ...
        lrl = list()
        lrl.append(
            ocp_lr.container_limit_range_item().cpu(min="200M", max="1200M").memory(min="528Mi", max="1Gi").build()
        )
        ...
        if not create_lr_response.metadata.name == NAME and not create_lr_response.metadata.namespace == NAMESPACE:
    ...
    def test_update_limit_range(self, ocp_lr):
        ...
        patch_set = [
            {"type": "Container", "max": {"cpu": "1200M", "memory": "1Gi"}, "min": {"memory": "528Mi", "cpu": "400M"}},
            {"type": "PersistentVolumeClaim", "max": {"storage": "10G"}},
        ]
