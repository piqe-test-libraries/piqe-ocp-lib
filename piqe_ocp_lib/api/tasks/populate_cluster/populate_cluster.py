...
class PopulateOcpCluster:
    ...
    def populate_cluster(self, filter="all"):
        ...
        if 'all' in filter:
            ...
            filtered_projects = [p for p in self.ocp_cluster_obj.projects if
                                 bool(set(filter) & set(p.project_labels.values()))]
        if not filtered_projects:
            logger.error("None of the filters you provided matched any project labels."
            ...
            def populate(project):
                ...
                for app in project.apps:
                    ...
                    logger.info('****** %s - Current app template used is: %s', threading.currentThread().getName(),
                                app.app_template)
                    ...
                    for i in range(app.app_count):
                        logger.info('----> %s - Now deploying: %s', threading.currentThread().getName(),
                                    app.app_template + '-' + str(i))
                        _, dc_names = self.app_obj.create_app_from_template(current_project, app.app_template, i,
                                                                            app.app_params)

                        if dc_names is None:
                            logger.error(" %s -Failed to deploy template: %s", threading.currentThread().getName(),
                                         app.app_template + '-' + str(i))
                            continue
                        ...
                        for dc in dc_names:
                            ...
                            if not dc_available:
                                logger.error(" %s - Timed out waiting for expected status conditions for "
                                             "deploymentconfig %s", threading.currentThread().getName(), dc)
                                sys.exit(1)
                            ...
                            if dc_ready:
                                ...
                                if dc_events:
                                    logger.debug("%s - Deploymentconfig events for %s:\n",
                                                 threading.currentThread().getName(), dc)
                                    ...
                                    for event in pod_events:
                                        logger.debug(
                                            ...
                                            % (event.involvedObject.namespace, event.involvedObject.name,
                                               event.firstTimestamp, event.message))
                                ...
                                if len(dc_pod) == 0:
                                    logger.error(" %s - No pods for deploymentconfig %s were found in the cluster.",
                                                 threading.currentThread().getName(), dc)
                            else:
                                logger.error("%s - Timed out waiting for the deploymentconfig %s to become ready."
                                             "Exiting now ...", threading.currentThread().getName(), dc)
                            ...
                            logger.info('%s - Now updating replicas for app %s', threading.currentThread().getName(),
                                        dc)
                            self.dc_obj.update_deployment_replicas(current_project, dc, app.app_replicas)
                            # Label the deployment configs of this app
                            logger.info('%s - Now labeling deploymentconfig %s', threading.currentThread().getName(),
                                        dc)
            ...
            if self.python_version <= ("2", "7"):
                ...
                for project in filtered_projects:
                    thread = threading.Thread(target=populate, name="Thread_{}".format(project.project_name),
                                              args=(project,))
            ...
            logger.info("Time taken to populate %s projects and %s apps is : %s ", len(filtered_projects),
    ...
    def longevity(self, duration, scale_replicas=5):
        ...
        while time.time() < end:
            ...
            if not dc_list.items:
                logger.error("No deployments labeled scalable=True were found"
                ...
                if bad_dcs:
                    for dc in bad_dcs:
                        ...
                        logger.error("The DeploymentConfig %s in namespace %s "
                                     "is in an invalid state."
                                     "See log entries below:",
                                     current_dc, current_namespace)
                    ...
                    logger.info('Modifying dc %s in project %s to scale to : %d replicas',
                                rand_dc_name, rand_dc_namespace, updated_replicas)
    ...
    def cleanup(self, filter="all"):
        ...
        if not filtered_projects:
            ...
            logger.info('Starting cleanup ... now deleting'
                        ' specified projects')
...
def argument_parser():
    ...
    parser = argparse.ArgumentParser(
        description='Process inputs for populate_ocp_cluster')

    parser.add_argument('-c', '--config',
                        action='store',
                        required=True,
                        help='YAML Config file that describes the cluster')
    ...
    parser.add_argument('-l', '--log-dir',
                        ...
                        required=False,
                        default="./logs",
                        help='log files dir path')
    parser.add_argument('-k8', '--kubeconfig',
                        action='store',
                        required=False,
                        help='OCP cluster kubeconfig file path')
    parser.add_argument('-n', '--log-file',
                        action='store',
                        dest="log_file",
                        required=False,
                        default=None,
    ...
    parser.add_argument('-s', '--span',
                        ...
                        default=3600,
                        type=int,
                        help='The span of the longevity test for scaling apps '
                             'in seconds')
    parser.add_argument('-r', '--replicas',
                        ...
                        default=5,
                        type=int,
                        help='The max number of replicas to scale apps '
                             'in the longevity test')
    parser.add_argument('-t', '--tests',
                        nargs='+',
                        choices=['populate', 'longevity', 'cleanup', 'all'],
                        default='all',
                        help='The specific tests to be run. Valid values are:'
                             'populate_cluster, longevity, cleanup or all.')
