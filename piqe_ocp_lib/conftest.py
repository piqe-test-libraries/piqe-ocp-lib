...
def pytest_addoption(parser):
    ...
    parser.addoption("--filter", action="append", type=str, default=[],
                     help="A filter to select which projects to deploy. This option"
                     "takes a single or comma separated list of labels.")
    parser.addoption("--span", action="store", type=int, default=60,
                     help="The duration for running the longevity in seconds")
    parser.addoption("--replicas", action="store", type=int, default=5,
    ...
    parser.addoption("--kubeconfig-3x", action="store", default=None,
                     help="The full path to the kubeconfig file to be used for ocp3x cluster")
    parser.addoption("--cluster-config", action="store", default=None,
                     help="The config yaml file describing the desired cluster layout")
    parser.addoption("--num-jenkins-jobs", action="store", default=None,
                     help="The number of jenkins job to create")
    parser.addoption("--num-locust-clients", action="store", default=None,
                     help="Number of concurrent Locust users. Only used together with --no-web")
    parser.addoption("--locust-hatch-rate", action="store", default=None,
